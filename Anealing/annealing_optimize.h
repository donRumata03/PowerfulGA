 ...
 * @return
 */
template<class GenomeElement, class GenomeGenerationFunctor, class EnergyFunctor,
		...
		const TemperatureChangingFunctor& temperature_changing_functor)
{
	...
	static_assert(std::is_same_v<decltype(mutation_functor(*(new Genome()))), Genome>, "False MutationFunctor's return type");
